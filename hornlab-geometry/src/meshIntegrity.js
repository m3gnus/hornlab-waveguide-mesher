import { triangleArea2, buildEdgeTopology as _buildEdgeTopology } from './edgeTopology.js';

const DEGENERATE_AREA_EPSILON = 1e-10;
const GEOMETRIC_DUPLICATE_EPSILON = 1e-6;

// Wrapper that returns only edgeUses (the local API used throughout this file).
function buildEdgeTopology(indices) {
  return _buildEdgeTopology(indices).edgeUses;
}
...
export function computeSignedVolume(vertices, indices) {
  let volume6 = 0;
