...
function calculateRossLength(constants, R, r0, k) {
  ...
  if (discriminant < 0) {
    throw new Error(
      'R is unreachable from r0 with these parameters (negative discriminant in R-OSSE length calculation)'
    );
  }
...
export function calculateROSSE(t, p, params) {
  ...
  let L;
  try {
    L = calculateRossLength(constants, R, r0, k);
  } catch (e) {
    console.error('R-OSSE length calculation failed:', e.message);
    return { x: NaN, y: NaN };
  }
  const { c1, c2, c3 } = constants;
  ...
  // Clamp mouthR at 0: for t > 1 the sqrt term grows and the expression
  // `R + L * (1 - sqrt(...))` can drop below zero, which is unphysical
  // (it would flip the horn inside-out). Callers normally pass t in [0, 1]
  // but tmax > 1 or numerical drift can overshoot.
