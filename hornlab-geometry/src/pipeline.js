...
function resolveBuildOptions(buildParams, options = {}) {
  const samplingMode = options.samplingMode
    ?? buildParams.samplingMode
    ?? buildParams.meshSamplingMode
    ?? (buildParams.athParitySampling === true ? 'ath-parity' : null);
  return {
    includeEnclosure: options.includeEnclosure ?? Number(buildParams.encDepth || 0) > 0,
    collectGroups: true,
    adaptivePhi: options.adaptivePhi ?? false,
    sliceMapMode: options.sliceMapMode ?? 'default',
    samplingMode
  };
}
...
function createGeometryShape(preparedParams, options = {}) {
  ...
  return {
    ...
    tessellation: {
      ...
      sliceMapMode: buildOptions.sliceMapMode || 'default',
      samplingMode: buildOptions.samplingMode || null
    }
  };
}
...
function buildMeshDataFromShape(shape, options = {}) {
  ...
  const buildOptions = resolveBuildOptions(buildParams, {
    ...
    sliceMapMode: options.sliceMapMode ?? shapeOptions.sliceMapMode,
    samplingMode: options.samplingMode ?? shapeOptions.samplingMode
  });
...
function buildGeometryMeshOutput(meshData) {
  return {
    ...
    angleList: Array.isArray(meshData.angleList) ? [...meshData.angleList] : null,
    sliceMap: Array.isArray(meshData.sliceMap) ? [...meshData.sliceMap] : null,
